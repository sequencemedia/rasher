import { SUPPORTS_ADD, SUPPORTS_REMOVE, SUPPORTS_ATTACH, SUPPORTS_DETACH, ATTACH, DETACH, FACADE } from '#bom/rasher';
...
export const attach = ATTACH === SUPPORTS_ADD ? attachEventListener : ATTACH === SUPPORTS_ATTACH ? attachEvent : notSupported;
export const detach = DETACH === SUPPORTS_REMOVE ? detachEventListener : DETACH === SUPPORTS_DETACH ? detachEvent : notSupported;
export const normalizeEvent = (e = window.event) => e;
export const eventTargetFor = e => {
  const E = normalizeEvent(e);
  return E.target || E.srcElement;
};
export const eventFacadeFor = FACADE === EVENT_LISTENER ? eventListenerFacade : FACADE === EVENT ? eventFacade : notSupported;