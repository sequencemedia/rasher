...
const has = (array, index) => typeof index === 'number' /* IS NUMBER */ && !(index < 0) /* LOWER BOUND (index not less than zero) */ && array.length > index /* UPPER BOUND (length more than index) */;
...
function listAll(subscriptionList) {
  return {
    ...
    indexOf(node) {
      return subscriptionList.findIndex(({
        node: n
      }) => node === n);
    },
    size() {
...
function delegateRasher() {
  ...
  return {
    delegate() {
      return delegate || (delegate = new Delegate());
    },
    query() {
...
function listenerRasher() {
  ...
  return {
    listener() {
      return listener || (listener = new Listener());
    },
    query() {
      return query || (query = new Query());
    },
