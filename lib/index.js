...
const find = (selector) => ({
  delegateTo: (delegate) => ({
    then: (callOut) => {
      ...
      if ((callOut || false).constructor !== Function) return { stop: () => false, list: () => [] }
      ...
      if (i < j) {
        do {
          ...
          callOut.call(subscriptionNode, delegateFacade(rasher, subscriptionList, subscriptionNode, selector), i, j)
        } while ((i = i + 1) < j)
      }
  ...
  then: (callOut) => {
    ...
    if (i < j) {
      do {
        const subscriptionNode = nodeList[i]
        callOut.call(subscriptionNode, listenerFacade(rasher, subscriptionList, subscriptionNode), i, j)
      } while ((i = i + 1) < j)
    }
